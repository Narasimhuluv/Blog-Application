      ...
      <>
        <div className="container w-full">
          <form action="" className="my-20" onSubmit={this.handleSubmit}>
            <label htmlFor="Comments" className="ml-6 font-extrabold">
              Comments
            </label>
            <input
              type="text"
              ...
              className="border-b block py-2 px-4 font-light focus:outline-none mt-6 comment"
            />
          </form>
        ...
        <div className="container w-8/12">
          {comments.map((eachComment) => (
            <div className="w-full my-8 " key={eachComment.author.username}>
              <article
                key={eachComment.id}
                className="flex items-start mb-3 w-5/12 "
              >
                <img
                  className="rounded-full image_size"
              ...
              {currentUser === eachComment.author.username ? (
                <div className="flex items-center my-4">
                  <p onClick={this.handleShowOPen}>
                    <i className="far fa-edit ml-5 cursor-pointer text-md text-yellow-700"></i>
                  </p>
                  <p onClick={() => this.handleCommentDelete(eachComment.id)}>
                    <i class="fas fa-trash ml-4 text-md text-red-400 cursor-pointer"></i>
                  </p>
                </div>
              ) : (
                ""
      ...
      <div className="w-full border bg-black relative">
        ...
        <div className="flex justify-center items-center">
          <form
            ...
            className="p-3 py-8 mt-2"
          >
            <label htmlFor="Comments" className="font-extrabold text-white">
              Update Comments
            </label>
            <input
              type="text"
              ...
              className="border-b block py-1 px-4 font-light focus:outline-none  comment"
            />
          </form>
        </div>
      </div>
