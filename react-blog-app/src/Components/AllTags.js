                    ...
                    <div className=" flex justify-between flex-wrap ">
                        {
                            filterTags.map((each) => (
                                ...
                                each === "" ? "" : <button key={each} className={tagName===each ? "border py-1 px-4 m-1 rounded-lg bg-yellow-400 text-white shadow-md text-sm" : "border py-1 px-4 m-1 rounded-lg shadow-md text-sm"}   onClick={() => this.props.handleTag(each)} data-value={each}>{(each)}</button>
                            ))
                        }
                    </div>
                </div>
