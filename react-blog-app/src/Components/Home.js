...
class Home extends React.Component {
  ...
  componentDidUpdate(_prevProps, prevState) {
    if (
      prevState.activeIndexPage !== this.state.activeIndexPage ||
      prevState.tagName !== this.state.tagName
    ) {
      this.FetchAllArticles();
    }
  }
      ...
      <>
        {this.props.isLogged ? (
          <AuthenticatedHome
            ...
            activeTab={activeTab}
            personalFeed={this.personalFeed}
            feeddata={feeddata}
            allArticles={allArticles}
            handleTag={this.handleTagArticle}
            updateCurrentIndex={this.updateCurrentPageIndex}
          />
        ) : (
          <UnAthenticatedHome
            {...this.state}
      ...
      <div>
        ...
        <div className="flex items-center global_eachTag">
          <div className="flex items-center">
            <NavLink to="/" activeClassName="text-yellow-700">
              <h2
                className="ml-20 text-lg font-extrabold mb-2"
                onClick={globalFeed}
              >
                Global Feed
              </h2>
            ...
            <NavLink to="/" activeClassName="text-yello-700">
              <h2
                className="ml-5 text-lg font-extrabold mb-2"
                onClick={personalFeed}
              >
                Personal Feed
              </h2>
            </NavLink>
          ...
          <h2 className="ml-5 text-yellow-600 text-xl font-extrabold mb-2">
            {activeTab && (
              <NavLink
                activeClassName="ml-2 text-yellow-600 text-xl font-extrabold mb-2"
                to="/"
              >
                #{activeTab}
              </NavLink>
        ...
        <div className="w-full flex mt-10">
          <div className="w-8/12 flex justify-center flex-wrap ">
            ...
            {activeTab === "feed"
              ? feeddata.map((each) => <FeedArticle each={each} />)
              : ""}
          </div>
...
function UnAthenticatedHome(props) {
  var { globalFeed, activeTab, allArticles, handleTag, updateCurrentIndex } =
    props;
  return (
    <section>
      <div>
        ...
        <div className="flex items-center global_eachTag">
          <NavLink to="/">
            <h2
              className="ml-20 text-xl font-extrabold mb-2"
              onClick={globalFeed}
            >
              Global Feed
            </h2>
          ...
          <h2 className="ml-5 text-yellow-600 text-xl font-extrabold mb-2">
            {activeTab && (
              <NavLink
                activeClassName="ml-5 text-yellow-600 text-xl font-extrabold mb-2"
                to="/"
              >
                #{activeTab}
              </NavLink>
...
function FeedArticle(props) {
  ...
  return (
    <>
      <article
        key={each.slug}
        className="border my-3 w-5/12 space-y-4 m-5 h-72 rounded-xl shadow-md relative overflow-hidden article"
